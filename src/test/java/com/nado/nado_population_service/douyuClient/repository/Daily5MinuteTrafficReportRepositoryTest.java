...
public class Daily5MinuteTrafficReportRepositoryTest {
	...
	public void saveSampleRecords(){
		...
		todaysRecordsOfTotal = repository.retieveRecordByDate(TODAY, MessageIntegrityStatuses.total);
		todaysRecordsOfBroken= repository.retieveRecordByDate(TODAY, MessageIntegrityStatuses.broken);
		yesterdaysRecordsOfTotal = repository.retieveRecordByDate(YESTERDAY, MessageIntegrityStatuses.total);
		yesterdaysRecordsOfBroken= repository.retieveRecordByDate(YESTERDAY, MessageIntegrityStatuses.broken);
	}
	/*
	...
	public void recordsHasCorrectValue(){
		for(String key: todaysRecordsOfTotal.keySet()){
			...
			assertTrue((correntValue+"").equals(repository.retieveRecord(TODAY, key, MessageIntegrityStatuses.total)));
			assertTrue((correntValue/100+"").equals(repository.retieveRecord(TODAY, key, MessageIntegrityStatuses.broken)));
			assertTrue(("1"+correntValue).equals(repository.retieveRecord(YESTERDAY, key, MessageIntegrityStatuses.total)));
			assertTrue(("1"+correntValue/100).equals(repository.retieveRecord(YESTERDAY, key, MessageIntegrityStatuses.broken)));
		}
	}
}
